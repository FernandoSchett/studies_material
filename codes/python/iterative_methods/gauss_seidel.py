...
Usage: 
	HowToExecute:   python3 gauss_seidel.py           
"""

def gs_info(x_new, x, iter):
    print('###############################################')
...
def gauss_seidel(A, b, x0, max_iter=100, tol=1e-6):
    ...
    for iter in range(max_iter):
        ...
        # Calculating
        for i in range(n):
            sigma_previous = sum(A[i][j] * x_new[j] for j in range(i))
            sigma_next = sum(A[i][j] * x[j] for j in range(i + 1, n))
            x_new[i] = (b[i] - sigma_previous - sigma_next) / A[i][i]
...
solution = gauss_seidel(A, b, x0, 100, 1e-6)
